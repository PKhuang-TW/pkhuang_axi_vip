...
class axi_master_driver extends axi_driver_base;
    ...
    extern virtual task listen_b_txn ();
...
task axi_master_driver::drive_w_txn();
    ...
    begin
        @(vif.mst_cb iff (w_q.size() > 0 && vif.mst_cb.ARESETn === 1'b1));
...
task axi_master_driver::listen_b_txn();
    axi_seq_item    txn, rsp;
    
    begin
        @ ( vif.mst_cb iff (vif.mst_cb.BVALID === 1'b1 && vif.mst_cb.BREADY === 1'b1) );
...
task axi_master_driver::drive_ar_txn ();
    ...
    begin
        ...
        @(vif.mst_cb iff vif.mst_cb.ARREADY === 1'b1);
        reset_ar_signal();
...
task axi_master_driver::drive_r_txn();
    ...
    begin
        @ ( vif.mst_cb iff ( vif.mst_cb.RVALID === 1'b1 && vif.mst_cb.RREADY === 1'b1 ) );
        vif.mst_cb.RREADY  <= 0;
